  ...
  <section ref="sectionRef" class="py-24 md:py-32 bg-[#FAFAF9] border-t border-[#C5BFB5]">
    <div class="max-w-[1280px] mx-auto px-6 md:px-8">
      <div class="mb-14 md:mb-16">
        <p class="font-mono text-[11px] uppercase text-[#5C554D] tracking-[0.2em] mb-4">Beneficios por recompra</p>
        <h2 class="font-display text-[clamp(2.5rem,6vw,4rem)] font-bold text-[#1C1917] leading-[0.92]">Crece con Nosotros</h2>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-14">
        <article v-for="(paso, index) in pasos" :key="paso.numero" class="bg-white border border-[#C5BFB5] p-6" :class="(visible || reducedMotion === 'reduce') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'" :style="{ transition: 'opacity 500ms ease-out, transform 500ms ease-out', transitionDelay: `${index * 120}ms` }">
          ...
          <p class="font-body text-[13px] text-[#5C554D] mt-2 leading-relaxed">{{ paso.desc }}</p>
        </article>
      </div>

      <div class="border border-[#C5BFB5] bg-white overflow-hidden">
        <div class="px-6 py-4 border-b border-[#C5BFB5] bg-[#FAFAF9]">
          <p class="font-mono text-[10px] uppercase tracking-[0.18em] text-[#5C554D]">Tabla de niveles comerciales</p>
        </div>
        <div class="hidden md:grid grid-cols-[2fr_1fr_2fr_1fr] bg-[#F5F0E8] text-[#1C1917] font-mono text-[10px] uppercase tracking-[0.2em] px-6 py-4 border-b border-[#C5BFB5]">
          <span>Rango</span>
        ...
        <div v-for="(rango, rowIndex) in rangos" :key="rango.nombre" class="grid grid-cols-1 md:grid-cols-[2fr_1fr_2fr_1fr] px-6 py-5 border-b border-[#C5BFB5] last:border-b-0 hover:bg-[#FAFAF9] transition-colors duration-300 gap-3 md:gap-0 items-center" :class="(visible || reducedMotion === 'reduce') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'" :style="{ transition: 'opacity 400ms ease-out, transform 400ms ease-out', transitionDelay: `${rowIndex * 100}ms` }">
          ...
          <div class="flex items-center gap-1.5">
            <div v-for="n in 4" :key="n" class="w-6 h-2 border border-[#C5BFB5] overflow-hidden">
          ...
          <span class="font-mono text-[11px] text-[#A16207] border border-[#C5BFB5] px-2 py-1 w-fit">Nivel {{ rango.nivel }}</span>
        </div>
      </div>
      ...
      <div class="mt-10" :class="(visible || reducedMotion === 'reduce') ? 'opacity-100' : 'opacity-0'" style="transition: opacity 500ms ease-out; transition-delay: 600ms">
        <NuxtLink to="/perfil" class="inline-flex items-center border border-[#C5BFB5] px-6 py-3 font-mono text-[11px] uppercase tracking-[0.15em] text-[#1C1917] hover:border-[#A16207] hover:text-[#A16207] transition-colors duration-300">
          Ver mis beneficios
        </NuxtLink>
      </div>
    </div>
