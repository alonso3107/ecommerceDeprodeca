  ...
  <section class="py-24 md:py-32 bg-[#FAFAF9] border-t border-[#D6D3D1]">
    <div class="max-w-[1280px] mx-auto px-6 md:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-14">
        <article v-for="paso in pasos" :key="paso.numero" class="bg-white border border-[#D6D3D1] p-6">
      ...
      <div class="border border-[#D6D3D1] bg-white overflow-hidden">
        ...
        <div v-for="rango in rangos" :key="rango.nombre" class="grid grid-cols-1 md:grid-cols-[2fr_1fr_2fr_1fr] px-6 py-5 border-b border-[#D6D3D1] last:border-b-0 hover:bg-[#F5F0E8] transition-colors duration-300 gap-3 md:gap-0 items-center">
          ...
          <div class="flex items-center gap-1.5">
            <div v-for="n in 4" :key="n" class="w-6 h-2 border border-[#D6D3D1]" :style="n <= rango.nivel ? { backgroundColor: '#A16207', borderColor: '#A16207' } : {}" />
          </div>
