    ...
    <div class="border border-borde bg-white overflow-hidden">
      <template v-if="pedidos.length">
        ...
        <NuxtLink v-for="pedido in pedidos.slice(0, 5)" :key="pedido.id" :to="`/pedidos/${pedido.id}`" class="grid grid-cols-[1fr_1fr_1fr_1fr] px-0 border-t border-borde hover:bg-fondo transition-colors duration-500">
          <div class="px-4 py-3 font-mono text-[13px] font-bold text-texto">#{{ pedido.id }}</div>
          <div class="px-4 py-3 font-mono text-[10px] text-texto-muted">{{ new Date(pedido.created_at).toLocaleDateString("es-PE") }}</div>
          <div class="px-4 py-3 font-mono text-[10px] text-texto">{{ money.format(pedido.total || 0) }}</div>
          <div class="px-4 py-3">
            <span class="inline-flex items-center gap-2 border px-2 py-1 font-mono text-[9px] uppercase tracking-[0.1em]" :style="{ borderColor: estadoColor(pedido.estado), color: estadoColor(pedido.estado) }">
      ...
      <div v-else class="min-h-[180px] flex items-center justify-center gap-3">
        ...
        <p class="font-mono text-[10px] uppercase tracking-[0.15em] text-texto-muted">Aun no tienes pedidos</p>
      </div>
    </div>
  </section>
</template>
