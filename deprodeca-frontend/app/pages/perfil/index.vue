      ...
      <section class="mb-12">
        ...
        <div class="border border-borde bg-white p-6 md:p-8">
          <div class="flex items-center justify-between mb-6 border-b border-borde pb-4">
            ...
            <button v-if="!editando" class="border border-borde px-4 py-2 font-mono text-[11px] uppercase tracking-[0.15em] hover:bg-fondo transition duration-500" @click="editando = true">Editar</button>
          </div>

          <div v-if="!editando" class="grid grid-cols-1 sm:grid-cols-2 border border-borde">
            <div class="p-5 border-b sm:border-r border-borde"><p class="font-mono text-[11px] uppercase text-texto-muted tracking-[0.15em] mb-1">Nombre</p><p class="font-body text-[18px] font-bold text-texto">{{ perfil.nombre }}</p></div>
            <div class="p-5 border-b border-borde"><p class="font-mono text-[11px] uppercase text-texto-muted tracking-[0.15em] mb-1">Empresa</p><p class="font-body text-[18px] font-bold text-texto">{{ perfil.empresa }}</p></div>
            <div class="p-5 border-b sm:border-b-0 sm:border-r border-borde"><p class="font-mono text-[11px] uppercase text-texto-muted tracking-[0.15em] mb-1">RUC</p><p class="font-body text-[18px] font-bold text-texto">{{ perfil.ruc }}</p></div>
            <div class="p-5"><p class="font-mono text-[11px] uppercase text-texto-muted tracking-[0.15em] mb-1">Email</p><p class="font-body text-[18px] font-bold text-texto">{{ perfil.email }}</p></div>
          </div>

          <form v-else @submit.prevent="guardarPerfil" class="space-y-4">
            <div><label class="block font-mono text-[12px] uppercase tracking-[0.15em] text-texto-muted mb-2">Nombre</label><input v-model="form.nombre" class="w-full border border-borde bg-white px-4 py-3 font-body focus:border-[#D4A017] focus:outline-none" required /></div>
            <div><label class="block font-mono text-[12px] uppercase tracking-[0.15em] text-texto-muted mb-2">Empresa</label><input v-model="form.empresa" class="w-full border border-borde bg-white px-4 py-3 font-body focus:border-[#D4A017] focus:outline-none" required /></div>
            <div><label class="block font-mono text-[12px] uppercase tracking-[0.15em] text-texto-muted mb-2">Telefono</label><input v-model="form.telefono" type="tel" class="w-full border border-borde bg-white px-4 py-3 font-body focus:border-[#D4A017] focus:outline-none" /></div>
            <div class="flex gap-0 pt-1">
              <button type="submit" :disabled="saving" class="bg-texto text-white px-6 py-3 font-mono text-[12px] uppercase tracking-[0.15em] hover:scale-[1.02] transition duration-500 disabled:opacity-60">{{ saving ? "Guardando" : "Guardar" }}</button>
              <button type="button" class="border border-borde border-l-0 px-6 py-3 font-mono text-[12px] uppercase tracking-[0.15em] text-texto-muted hover:bg-fondo transition duration-500" @click="editando = false">Cancelar</button>
            </div>
          </form>
        </div>
      </section>
